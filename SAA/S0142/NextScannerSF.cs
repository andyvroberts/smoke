...
namespace S0142
{
    ...
    public class NextScannerSF
    {
        [Function("NextScannerSF")]
        public static async Task Run(
            [TimerTrigger("0 40,45,50 5 * * *")] TimerInfo scanTimer,
            FunctionContext context)
        {
            var _logger = context.GetLogger(nameof(NextScannerSF));

            var configTab = GetConfigTableClient("AcquisitionConfig");
            var confRow = configTab.GetEntity<ConfigTable>(Constants.ConfigPK, Constants.ConfigFinalInitialRK).Value;
